...
ras_gem_2005_rcp45 <- raster('wy_projections/ss/ss_ann_HadGEM2-ES365_rcp45_1999.tif', band = 7)
ras_gem_2005_rcp85 <- raster('wy_projections/ss/ss_ann_HadGEM2-ES365_rcp85_1999.tif', band = 7)
ras_gem_2015_rcp45 <- raster('wy_projections/ss/ss_ann_HadGEM2-ES365_rcp45_1999.tif', band = 17)
ras_gem_2015_rcp85 <- raster('wy_projections/ss/ss_ann_HadGEM2-ES365_rcp85_1999.tif', band = 17)

#set ss values to min 0 and max 50
ras_gem_2005_rcp45[ras_gem_2005_rcp45 > 50] <- 50
ras_gem_2005_rcp45[ras_gem_2005_rcp45 < 0] <- 0
ras_gem_2005_rcp85[ras_gem_2005_rcp85 > 50] <- 50
ras_gem_2005_rcp85[ras_gem_2005_rcp85 < 0] <- 0
ras_gem_2015_rcp45[ras_gem_2015_rcp45 > 50] <- 50
ras_gem_2015_rcp45[ras_gem_2015_rcp45 < 0] <- 0
ras_gem_2015_rcp85[ras_gem_2015_rcp85 > 50] <- 50
ras_gem_2015_rcp85[ras_gem_2015_rcp85 < 0] <- 0
...
#change layer names
names(ras_gem_2005_rcp45) <- c('ss', 'rain', 'vpd', 'pdsi', 'lc')
names(ras_gem_2005_rcp85) <- c('ss', 'rain', 'vpd', 'pdsi', 'lc')
names(ras_gem_2015_rcp45) <- c('ss', 'rain', 'vpd', 'pdsi', 'lc')
names(ras_gem_2015_rcp85) <- c('ss', 'rain', 'vpd', 'pdsi', 'lc')
...
rm(dat)
dat2 <- dat2[, colnames(dat2) %in% c('year', 'ss', 'lc', 'pdsi_mar_apr_min', 'rain_mar_may', 'vpd_tavg_mean_jan_apr')]
colnames(dat2) <- c('year', 'ss_dlc', 'lc_dlc', 'pdsi_dlc', 'vpd_dlc', 'rain_dlc')
dat2 <- dat2[, c('year', 'ss_dlc', 'rain_dlc', 'vpd_dlc', 'pdsi_dlc', 'lc_dlc')]
...
dat_gem_2005_rcp45 <- cbind(dat_gem_2005_rcp45, dat_2005[, c('ss_dlc', 'rain_dlc', 'vpd_dlc', 'pdsi_dlc', 'lc_dlc')])
dat_gem_2005_rcp85 <- cbind(dat_gem_2005_rcp85, dat_2005[, c('ss_dlc', 'rain_dlc', 'vpd_dlc', 'pdsi_dlc', 'lc_dlc')])
dat_gem_2015_rcp45 <- cbind(dat_gem_2015_rcp45, dat_2015[, c('ss_dlc', 'rain_dlc', 'vpd_dlc', 'pdsi_dlc', 'lc_dlc')])
dat_gem_2015_rcp85 <- cbind(dat_gem_2015_rcp85, dat_2015[, c('ss_dlc', 'rain_dlc', 'vpd_dlc', 'pdsi_dlc', 'lc_dlc')])
...
dat_samples$vpd_dlc <- round(dat_samples$vpd_dlc, 2)
dat_samples$ss <- round(dat_samples$ss, 2)

#remove id col
#dat_samples <- dat_samples[, !(colnames(dat_samples) %in% 'ID')]

#write to csv
write.csv(dat_samples, 'output/spring_scale/bit_ss_comparison.csv', row.names = F)

#plot
ggplot(dat_samples) +
  geom_point(aes(x = rain, y = ss, col = 'red')) +
  geom_point(aes(x = rain_dlc, y = ss_dlc, col = 'blue')) +
  theme_bw() + xlab('Rain Mar-May (mm)') + ylab('Spring Scale (Days)') +
  ggtitle('BIT vs. DLC Comparisons 2005 & 2015: Rain') + 
  scale_color_discrete(name = 'Scenario', labels = c('Observed', 'Projected'))

ggsave('output/final_plots/pirgd_ss/bit_ss_rain.png', width = 12,
       height = 8)

 ggplot(dat_samples) +
  geom_point(aes(x = vpd, y = ss, col = 'red')) +
  geom_point(aes(x = vpd_dlc, y = ss_dlc, col = 'blue')) +
  theme_bw() + xlab('Mean VPD Jan-Apr') + ylab('Spring Scale (Days)') +
  ggtitle('BIT vs. DLC Comparisons 2005 & 2015: VPD') + 
  scale_color_discrete(name = 'Scenario', labels = c('Observed', 'Projected'))
 
 ggsave('output/final_plots/pirgd_ss/bit_ss_vpd.png', width = 12,
        height = 8)

ggplot(dat_samples) +
  geom_point(aes(x = pdsi, y = ss, col = 'red')) +
  geom_point(aes(x = pdsi_dlc, y = ss_dlc, col = 'blue')) +
  theme_bw() + xlab('Min PDSI Mar-Apr') + ylab('Spring Scale (Days)') +
  ggtitle('BIT vs. DLC Comparisons 2005 & 2015: VPD') + 
  scale_color_discrete(name = 'Scenario', labels = c('Observed', 'Projected'))

ggsave('output/final_plots/pirgd_ss/bit_ss_pdsi.png', width = 12,
       height = 8)
...
ggsave('output/final_plots/pirgd_ss/bit_ss_evergreen_rain.png', width = 12,
       height = 8)
...
ggsave('output/final_plots/pirgd_ss/bit_ss_shrub_rain.png', width = 12,
       height = 8)
...
ggsave('output/final_plots/pirgd_ss/bit_ss_herb_rain.png', width = 12,
       height = 8)
...
ggplot(dat_samples[dat_samples$lc == 'evergreen',]) +
  ...
  geom_point(aes(x = vpd_dlc, y = ss_dlc, col = 'blue')) +
  theme_bw() + xlab('Mean VPD Jan-Apr') + ylab('Spring Scale (Days)') +
  ggtitle('BIT vs. DLC Comparisons 2005 & 2015: VPD in Evergreen') + 
  scale_color_discrete(name = 'Scenario', labels = c('Observed', 'Projected'))

ggsave('output/final_plots/pirgd_ss/bit_ss_evergreen_vpd.png', width = 12,
       height = 8)
...
ggplot(dat_samples[dat_samples$lc == 'shrub',]) +
  ...
  geom_point(aes(x = vpd_dlc, y = ss_dlc, col = 'blue')) +
  theme_bw() + xlab('Mean VPD Jan-Apr') + ylab('Spring Scale (Days)') +
  ggtitle('BIT vs. DLC Comparisons 2005 & 2015: VPD in Shrub') + 
  scale_color_discrete(name = 'Scenario', labels = c('Observed', 'Projected'))

ggsave('output/final_plots/pirgd_ss/bit_ss_shrub_vpd.png', width = 12,
       height = 8)
...
ggplot(dat_samples[dat_samples$lc == 'herb',]) +
  ...
  geom_point(aes(x = vpd_dlc, y = ss_dlc, col = 'blue')) +
  theme_bw() + xlab('Mean VPD Jan-Apr') + ylab('Spring Scale (Days)') +
  ggtitle('BIT vs. DLC Comparisons 2005 & 2015: VPD in Herb') + 
  scale_color_discrete(name = 'Scenario', labels = c('Observed', 'Projected'))

ggsave('output/final_plots/pirgd_ss/bit_ss_Herb_vpd.png', width = 12,
       height = 8)
