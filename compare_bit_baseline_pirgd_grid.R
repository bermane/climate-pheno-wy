#this code compares fitted model outputs for PIRGd against BIT data from the middle model
...
vals <- data.frame(id = dat2$id, year = dat2$year, fitted = fitted(m_pirgd) %>% round)
...
#load BIT values
pred_rcp45 <- brick('wy_projections/pirgd/pirgd_ann_HadGEM2-ES365_rcp45_1999.tif') %>%
  .[[3:20]]

pred_rcp85 <- brick('wy_projections/pirgd/pirgd_ann_HadGEM2-ES365_rcp85_1999.tif') %>%
  .[[3:20]]
...
#set up output plot
jpeg('/Volumes/SSD/climate_effects/output/final_plots/resid_size_fitted_bit_pirgd.jpeg', width = 715*2, height = 495*2)

#plot grid points
plot(lc, legend=FALSE, col=c("coral3", "papayawhip", "forestgreen"), xaxt='n', yaxt='n',
     main = "Average PIRGd Residual Size on Sampling Grid Fitted vs. BIT \n Black Pos Blue Neg") 
par(xpd=TRUE)
...
plot(grid_pos, pch = 19, cex = abs(grid@data$resid)*0.5, add = T, 
     col = rgb(red = 0, green = 0, blue = 0, alpha = 0.75))
plot(grid_neg, pch = 19, cex = abs(grid@data$resid)*0.5, add = T, 
