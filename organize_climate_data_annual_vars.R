  ...
  for(yr in years){
    ...
    for(r in rcp){
      ...
      for(y in unique(dat$year)){
        ...
        tmax <- tmax_yr[, which(ts_yr == str_c(y, '0301')):which(ts_yr == str_c(y, '0531'))]
        rain[tmax <= tmax_thresh] <- 0
        ...
        if(y != start_yr){
          ...
          tmax <- tmax_yr[, which(ts_yr == str_c(y-1, '1001')):which(ts_yr == str_c(y, '0430'))]
          snow[tmax > tmax_thresh] <- 0
