...
ggplot(data = dat_raw2[dat_raw2$rcp == 'rcp45',], aes(x = lc, color = model, y = value)) +
  geom_boxplot() + 
  colScale + xlab('Landcover') + ylab('PIRGd (Date)') +
  ggtitle('BIT Comparison RCP 4.5') + theme_bw() + theme_e +
  scale_y_continuous(breaks = c(91, 121, 152), labels = c('Apr 1', 'May 1', 'Jun 1'))

ggsave('output/final_plots/bit_comparisons_rcp45_pirgd.png',
...
ggsave('output/final_plots/bit_comparisons_rcp85_pirgd.png',
       width = 9, height = 5, unit = "in")
