...
#add log gdd and log rain as covariates
dat2$log_gdd_jan_apr <- log(dat2$gdd_jan_apr)
#dat2$log_gdd_jan_apr[dat2$log_gdd_jan_apr < 1] <- 1

dat2$log_rain_mar_may <- log(dat2$rain_mar_may)
...
for(col_id in 7:37){
  ...
  #set up output plot
  jpeg(str_c('output/pirgd/log_gdd_log_rain/norm_equal_var_test_plots/' ,vari, '.jpeg'), width = 900, height = 600)
...
#don't use quadratics for log variables
quad_var <- quad_var[!(quad_var %in% c('log_gdd_jan_apr', 'log_rain_mar_may'))]
...
#calc col means and SD
dat2_mean <- colMeans(dat2[,7:37])
dat2_sd <- apply(dat2[,7:37], 2, sd)
...
write.csv(cor_df, file = 'output/pirgd/log_gdd_log_rain/full correlation results - 2021-01-09.csv')
...
write.csv(cor_df, file = 'output/pirgd/log_gdd_log_rain/significant correlation results - 2021-01-09.csv')
...
#write to csv file
write.csv(uni_df, file = 'output/pirgd/log_gdd_log_rain/univariate results - 2021-01-09.csv')
...
combos <- as.data.frame(expand.grid(log_gdd_jan_apr = 0:1, gdd_jan_apr = 0:1))
...
combos <- as.data.frame(expand.grid(twi = 0:1, pdsi_mar_apr_min = 0:1, log_rain_mar_may = 0:1,
                                    snow_oct_apr = 0:1))
...
m_temp <- lme(pirgd ~ lc + log_gdd_jan_apr, 
              random = ~ 1 | id, data = dat3, method = 'ML')

m_precip <- lme(pirgd ~ lc + poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                  snow_oct_apr, random = ~ 1 | id, data = dat3, method = 'ML')

m_temp_precip <- lme(pirgd ~ lc + log_gdd_jan_apr +
                       poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                      snow_oct_apr, 
                     random = ~ 1 | id, data = dat3, method = 'ML')
...
m_base <- lme(pirgd ~ lc + log_gdd_jan_apr +
                poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                snow_oct_apr, 
              random = ~ 1 | id, data = dat3, method = 'ML')

m_int_gdd_rain <- lme(pirgd ~ lc + log_gdd_jan_apr +
                   poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                   snow_oct_apr + 
                   log_gdd_jan_apr:log_rain_mar_may, 
                 random = ~ 1 | id, data = dat3, method = 'ML')

m_int_gdd_snow <- lme(pirgd ~ lc + log_gdd_jan_apr +
                        poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                        snow_oct_apr + 
                        log_gdd_jan_apr:snow_oct_apr, 
                      random = ~ 1 | id, data = dat3, method = 'ML')

m_int_rain_snow <- lme(pirgd ~ lc + log_gdd_jan_apr +
                        poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                        snow_oct_apr + 
                        log_rain_mar_may:snow_oct_apr, 
                      random = ~ 1 | id, data = dat3, method = 'ML')

m_int_pdsi_lc <- lme(pirgd ~ lc + log_gdd_jan_apr +
                   poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                   snow_oct_apr + 
                   poly(pdsi_mar_apr_min, 2, raw = TRUE):lc, 
                 random = ~ 1 | id, data = dat3, method = 'ML')

m_int_gdd_rain_pdsi_lc <- lme(pirgd ~ lc + log_gdd_jan_apr +
               poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
               snow_oct_apr +
               log_gdd_jan_apr:log_rain_mar_may +
               poly(pdsi_mar_apr_min, 2, raw = TRUE):lc, 
...
m_int_gdd_snow_pdsi_lc <- lme(pirgd ~ lc + log_gdd_jan_apr +
                                poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                                snow_oct_apr +
                                log_gdd_jan_apr:snow_oct_apr +
                                poly(pdsi_mar_apr_min, 2, raw = TRUE):lc, 
...
m_int_rain_snow_pdsi_lc <- lme(pirgd ~ lc + log_gdd_jan_apr +
                                poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                                snow_oct_apr +
                                log_rain_mar_may:snow_oct_apr +
                                poly(pdsi_mar_apr_min, 2, raw = TRUE):lc, 
...
#create output df
int_df <- data.frame(model = character(), aic = numeric())

#rbind all results
int_df <- rbind(int_df, data.frame(model = 'm_base', aic = AIC(m_base)),
                data.frame(model = 'm_int_gdd_rain', aic = AIC(m_int_gdd_rain)),
                data.frame(model = 'm_int_gdd_snow', aic = AIC(m_int_gdd_snow)),
                data.frame(model = 'm_int_rain_snow', aic = AIC(m_int_rain_snow)),
                data.frame(model = 'm_int_pdsi_lc', aic = AIC(m_int_pdsi_lc)),
                data.frame(model = 'm_int_gdd_rain_pdsi_lc', aic = AIC(m_int_gdd_rain_pdsi_lc)),
                data.frame(model = 'm_int_gdd_snow_pdsi_lc', aic = AIC(m_int_gdd_snow_pdsi_lc)),
                data.frame(model = 'm_int_rain_snow_pdsi_lc', aic = AIC(m_int_rain_snow_pdsi_lc)))

#######################################
###TEST VARIANCE STRUCTURES FOR RAIN###
#######################################

#leave this out for now!!!!

#three additional variance options since rain is not categorical

# #varExp and varConstPower
# v_pow <- varPower(form =~ c(rain_mar_may))
# v_exp <- varExp(form =~ c(rain_mar_may))
# v_cp <- varConstPower(form =~ c(rain_mar_may))
# 
# #fit models with differing variance structures
# m_pow <- lme(pirgd ~ lc + log_gdd_jan_apr +
#                poly(pdsi_mar_apr_min, 2, raw = TRUE) + poly(rain_mar_may, 2, raw = TRUE) + 
#                snow_oct_apr +
#                log_gdd_jan_apr:poly(rain_mar_may, 2, raw = TRUE) +
#                poly(pdsi_mar_apr_min, 2, raw = TRUE):lc, 
#              random = ~ 1 | id, data = dat3,
#              weights = v_pow, method = 'ML')
# 
# m_exp <- lme(pirgd ~ lc + log_gdd_jan_apr +
#                poly(pdsi_mar_apr_min, 2, raw = TRUE) + poly(rain_mar_may, 2, raw = TRUE) + 
#                snow_oct_apr +
#                log_gdd_jan_apr:poly(rain_mar_may, 2, raw = TRUE) +
#                poly(pdsi_mar_apr_min, 2, raw = TRUE):lc, 
#              random = ~ 1 | id, data = dat3,
#              weights = v_exp, method = 'ML')
# 
# m_cp <- lme(pirgd ~ lc + log_gdd_jan_apr +
#                poly(pdsi_mar_apr_min, 2, raw = TRUE) + poly(rain_mar_may, 2, raw = TRUE) + 
#                snow_oct_apr +
#                log_gdd_jan_apr:poly(rain_mar_may, 2, raw = TRUE) +
#                poly(pdsi_mar_apr_min, 2, raw = TRUE):lc, 
#              random = ~ 1 | id, data = dat3,
#              weights = v_cp, method = 'ML')
# 
# #check AIC of two var structures and base model
# AIC(m_all)
# AIC(m_pow)
# AIC(m_exp)
# AIC(m_cp)
...
m_final <- lme(pirgd ~ lc + log_gdd_jan_apr +
                 poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                 snow_oct_apr +
                 log_rain_mar_may:snow_oct_apr +
                 poly(pdsi_mar_apr_min, 2, raw = TRUE):lc, 
...
#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/m_final_resid_dist.jpeg', width = 900, height = 600)
...
#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/m_final_resid_vs_fitted.jpeg', width = 900, height = 600)
...
#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/m_final_resid_lc.jpeg', width = 900, height = 600)
...
vars <- c('log_gdd_jan_apr', 'pdsi_mar_apr_min', 'log_rain_mar_may',
          'snow_oct_apr')
...
for(var in vars){
  ...
  #set up output plot
  jpeg(str_c('output/pirgd/log_gdd_log_rain/model_validation/m_final_resid_', var, '.jpeg'), width = 900, height = 600)
...
m_final_lme4 <- lmer(pirgd ~ lc + log_gdd_jan_apr +
                       poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may +
                       snow_oct_apr +
                       log_rain_mar_may:snow_oct_apr +
...
AIC(m_final)
performance::model_performance(m_final)
...
#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/predicted_vs_observed_pirgd.jpeg', width = 600, height = 600)
...
#recreate variable for gdd to use as log

m_original <- lme(pirgd ~ lc + log(gdd_jan_apr) +
...
#check performace against results with rescaled data
performance::model_performance(m_original)

#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/variable_effect_size.jpeg', width = 1600, height = 902)
...
vars_effect <- c('lc', 'snow_oct_apr', 'pdsi_mar_apr_min', 'rain_mar_may',
                 'gdd_jan_apr')
...
#plot
plot(effects::predictorEffect(vars_effect[1], m_original, residuals = T,
                              xlevels=list(pdsi_mar_apr_min = c(-2, -0.4, 1.4))),
...
#plot with residuals
plot(effects::Effect(vars_effect[1], m_original, residuals = T,
                              xlevels=list(pdsi_mar_apr_min = c(-2, 0, 2))),
     axes = list(y = list(lim = c(60, 300), lab = 'PIRGd (DOY)'),
                 x = list(lc = list(lab = 'Landcover'))),
     partial.residuals=list(smooth=TRUE,span=0.9),
     main = 'Landcover Predictor Effect',
     lines=list(multiline=TRUE), confint=list(style="auto"))
...
plot(effects::predictorEffect(vars_effect[2], m_original,
                              xlevels=list(rain_mar_may = c(90, 150, 200))),
     axes = list(y = list(lim = c(60, 300), lab = 'PIRGd (DOY)'),
...
#plot more extremes
plot(effects::predictorEffect(vars_effect[2], m_original,
                              xlevels=list(rain_mar_may = c(90, 150, 200, 300, 500, 700))),
     axes = list(y = list(lim = c(60, 300), lab = 'PIRGd (DOY)'),
                 x = list(snow_oct_apr = list(lab = 'Snow Oct-Apr'))),
     main = 'Snow Oct-Apr Predictor Effect',
     lines=list(multiline=TRUE), confint=list(style="auto"))
...
#plot with residuals
plot(effects::Effect(c('gdd_jan_apr', 'rain_mar_may'), m_original, residuals = T,
                              xlevels=list(rain_mar_may = c(100, 150, 200))),
     axes = list(y = list(lim = c(60, 300), lab = 'PIRGd (DOY)'),
                 x = list(gdd_jan_apr = list(lab = 'GDD Jan-Apr'))),
     partial.residuals=list(smooth=TRUE,span=0.9),
     main = 'GDD Jan-Apr Predictor Effect',
     lines=list(multiline=F), confint=list(style="auto"))
...
#code for var 4
#calc quantiles and mean and use nice rounded values
quantile(dat2$snow_oct_apr, 0.25)
mean(dat2$snow_oct_apr)
quantile(dat2$snow_oct_apr, 0.75)

#plot
plot(effects::predictorEffect(vars_effect[4], m_original,
                              xlevels=list(snow_oct_apr = c(35, 100, 140))),
     axes = list(y = list(lim = c(60, 300), lab = 'PIRGd (DOY)'),
...
#plot with extremes
plot(effects::predictorEffect(vars_effect[4], m_original,
                              xlevels=list(snow_oct_apr = c(35, 100, 140, 200, 300, 400))),
     axes = list(y = list(lim = c(60, 300), lab = 'PIRGd (DOY)'),
                 x = list(rain_mar_may = list(lab = 'Rain Mar-May'))),
     main = 'Rain Mar-May Predictor Effect',
     lines=list(multiline=TRUE), confint=list(style="auto"))
...
#plot with residuals
plot(effects::Effect(c('rain_mar_may', 'gdd_jan_apr'), m_original, residuals = T,
...
#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/pirgd_vs_landcover.jpeg', width = 900, height = 600)
...
#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/pirgd_vs_gdd.jpeg', width = 900, height = 600)

#plot pirgd vs gdd
plot(x = dat2$gdd_jan_apr, y = dat2$pirgd)

#dev.off
dev.off()

#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/pirgd_vs_log_gdd.jpeg', width = 900, height = 600)

#plot pirgd vs log gdd
plot(x = log(dat2$gdd_jan_apr), y = dat2$pirgd)

#dev.off
dev.off()

#set up output plot
jpeg('output/pirgd/log_gdd_var_rain/model_validation/pirgd_vs_rain.jpeg', width = 900, height = 600)

#plot pirgd vs log rain
plot(x = dat2$rain_mar_may, y = dat2$pirgd)

#dev.off
dev.off()

#set up output plot
jpeg('output/pirgd/log_gdd_var_rain/model_validation/pirgd_vs_log_rain.jpeg', width = 900, height = 600)

#plot pirgd vs log rain
plot(x = log(dat2$rain_mar_may), y = dat2$pirgd)
...
#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/resid_size_sampling_grid.jpeg', width = 715*2, height = 495*2)

#plot grid points using residual size
plot(lc, legend=FALSE, col=c("coral3", "papayawhip", "forestgreen"), xaxt='n', yaxt='n',
     main = "Average PIRGd Residual Size on Sampling Grid \nBlack Pos Blue Neg") 
par(xpd=TRUE)
...
plot(grid_pos, pch = 19, cex = abs(grid@data$resid)*50, add = T, 
     col = rgb(red = 0, green = 0, blue = 0, alpha = 0.75))
plot(grid_neg, pch = 19, cex = abs(grid@data$resid)*50, add = T, 
...
#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/pdsi_pirgd_lc.jpeg', width = 715, height = 495)
...
#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/pdsi_pirgd_elev.jpeg', width = 715, height = 495)
...
#create index first
index <- c('(Intercept)', 'Herb', 'Evergreen', 'log(GDD)', 'log(Rain)',
           'PDSI', 'PDSI^2', 'Snow', 'log(Rain):Snow',
           'Herb:PDSI',
           'Herb:PDSI^2', 'Evergreen:PDSI',
           'Evergreen:PDSI^2')

names(index) <- c('(Intercept)', 'lcherb', 'lcevergreen', 'log_gdd_jan_apr',
                  'log_rain_mar_may',
                  'poly(pdsi_mar_apr_min, 2, raw = TRUE)1', 'poly(pdsi_mar_apr_min, 2, raw = TRUE)2',
                  'snow_oct_apr',
                  'log_rain_mar_may:snow_oct_apr',
                  'lcherb:poly(pdsi_mar_apr_min, 2, raw = TRUE)1',
                  'lcherb:poly(pdsi_mar_apr_min, 2, raw = TRUE)2',
                  'lcevergreen:poly(pdsi_mar_apr_min, 2, raw = TRUE)1',
                  'lcevergreen:poly(pdsi_mar_apr_min, 2, raw = TRUE)2')
...
m_df$category[m_df$term %in% c('log(GDD)')] <- 'Temperature'
m_df$category[m_df$term %in% c('log(Rain)', 'PDSI',
                               'PDSI^2', 'Snow')] <- 'Moisture'
...
#set up output plot
jpeg('output/pirgd/log_gdd_log_rain/model_validation/effect_dot_whisker.jpeg', width = 715, height = 495)
...
dotwhisker::dwplot(m_df,
  ...
  theme(legend.position = (c(0.72, 0.03)),
        ...
        plot.title = element_text(size = 20), axis.text.x = element_text(size = 15),
