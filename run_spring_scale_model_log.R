...
#add log gdd log rain, and log vpd as covariates
dat2$log_gdd_jan_apr <- log(dat2$gdd_jan_apr)
dat2$log_rain_mar_may <- log(dat2$rain_mar_may)
dat2$log_vpd_tavg_mean_jan_apr <- log(dat2$vpd_tavg_mean_jan_apr)
...
for(col_id in 7:38){
  ...
  #set up output plot
  jpeg(str_c('output/spring_scale/log/norm_equal_var_test_plots/' ,vari, '.jpeg'), width = 900, height = 600)
...
for(col_id in 7:38){
  ...
  #rain_mar_may quadratic does not converge in this iteration
 # if(vari != 'rain_mar_may'){
    #fit a linear mixed model with landcover, random effect, and variable
...
#make log variables linear
quad_var <- quad_var[!(quad_var %in% c('log_gdd_jan_apr', 'log_rain_mar_may', 'log_vpd_tavg_mean_jan_apr'))]
...
dat2_mean <- colMeans(dat2[,7:38])
dat2_sd <- apply(dat2[,7:38], 2, sd)
...
write.csv(cor_df, file = 'output/spring_scale/log/full correlation results - 2020-12-15.csv')
...
write.csv(cor_df, file = 'output/spring_scale/log/significant correlation results - 2020-12-15.csv')
...
#write to csv file
write.csv(uni_df, file = 'output/spring_scale/log/univariate results - 2020-12-15.csv')
...
combos <- as.data.frame(expand.grid(log_gdd_jan_apr = 0:1, chill_jan_may = 0:1))
...
combos <- as.data.frame(expand.grid(pdsi_mar_apr_min = 0:1, log_rain_mar_may = 0:1,
                                    snow_oct_apr = 0:1, log_vpd_tavg_mean_jan_apr = 0:1))
...
m_precip <- lme(ss ~ lc + poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                  log_vpd_tavg_mean_jan_apr, random = ~ 1 | id, data = dat3, method = 'ML')
...
m_base <- lme(ss ~ lc + poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                log_vpd_tavg_mean_jan_apr, random = ~ 1 | id, data = dat3, method = 'ML')

m_int_pdsi_rain <- lme(ss ~ lc + poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                   log_vpd_tavg_mean_jan_apr + poly(pdsi_mar_apr_min, 2, raw = TRUE):log_rain_mar_may, 
                 random = ~ 1 | id, data = dat3, method = 'ML')

m_int_vpd_rain <- lme(ss ~ lc + poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                   log_vpd_tavg_mean_jan_apr + log_vpd_tavg_mean_jan_apr:log_rain_mar_may, 
                 random = ~ 1 | id, data = dat3, method = 'ML')

m_int_vpd_pdsi <- lme(ss ~ lc + poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                   log_vpd_tavg_mean_jan_apr + log_vpd_tavg_mean_jan_apr:poly(pdsi_mar_apr_min, 2, raw = TRUE), 
                 random = ~ 1 | id, data = dat3, method = 'ML')

m_int_pdsi_rain_vpd_rain <- lme(ss ~ lc + poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                      log_vpd_tavg_mean_jan_apr + poly(pdsi_mar_apr_min, 2, raw = TRUE):log_rain_mar_may +
                      log_vpd_tavg_mean_jan_apr:log_rain_mar_may, 
                    random = ~ 1 | id, data = dat3, method = 'ML')
...
int_df <- rbind(int_df, data.frame(model = 'm_base', aic = AIC(m_base)),
                data.frame(model = 'm_int_pdsi_rain', aic = AIC(m_int_pdsi_rain)),
                data.frame(model = 'm_int_vpd_rain', aic = AIC(m_int_vpd_rain)),
                data.frame(model = 'm_int_vpd_pdsi', aic = AIC(m_int_vpd_pdsi)),
                data.frame(model = 'm_int_pdsi_rain_vpd_rain', aic = AIC(m_int_pdsi_rain_vpd_rain)))
...
m_final <- lme(ss ~ lc + poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                 log_vpd_tavg_mean_jan_apr + poly(pdsi_mar_apr_min, 2, raw = TRUE):log_rain_mar_may +
                 log_vpd_tavg_mean_jan_apr:log_rain_mar_may, 
...
#set up output plot
jpeg('output/spring_scale/log/model_validation/resid_dist.jpeg', width = 900, height = 600)
...
#set up output plot
jpeg('output/spring_scale/log/model_validation/m_final_resid_vs_fitted.jpeg', width = 900, height = 600)
...
#set up output plot
jpeg('output/spring_scale/log/model_validation/m_final_resid_lc.jpeg', width = 900, height = 600)
...
vars <- c('pdsi_mar_apr_min', 'log_rain_mar_may',
          'log_vpd_tavg_mean_jan_apr')

#loop through
for(var in vars){
  
  #set up output plot
  jpeg(str_c('output/spring_scale/log/model_validation/m_final_resid_', var, '.jpeg'), width = 900, height = 600)
...
m_final_lme4 <- lmer(ss ~ lc + poly(pdsi_mar_apr_min, 2, raw = TRUE) + log_rain_mar_may + 
                       log_vpd_tavg_mean_jan_apr + poly(pdsi_mar_apr_min, 2, raw = TRUE):log_rain_mar_may +
                       log_vpd_tavg_mean_jan_apr:log_rain_mar_may +
                       (1 | id), dat3)
...
#set up output plot
jpeg('output/spring_scale/log/model_validation/predicted_vs_observed_ss.jpeg', width = 600, height = 600)
...
m_original <- lme(ss ~ lc + poly(pdsi_mar_apr_min, 2, raw = TRUE) + log(rain_mar_may) + 
...
#set up output plot
jpeg('output/spring_scale/log/model_validation/variable_effect_size.jpeg', width = 1600, height = 902)
...
#plot with extremes
plot(effects::predictorEffect(vars_effect[2], m_original,
                              xlevels=list(rain_mar_may = c(90, 150, 200, 400, 600))),
     axes = list(y = list(lim = c(0, 60), lab = 'Spring Scale'),
                 x = list(pdsi_mar_apr_min = list(lab = 'Min PDSI Mar-Apr'))),
     main = 'Min PDSI Mar-Apr Predictor Effect',
     lines=list(multiline=TRUE), confint=list(style="auto"))
...
#set up output plot
jpeg('output/spring_scale/log/model_validation/ss_vs_landcover.jpeg', width = 900, height = 600)
...
#set up output plot
jpeg('output/spring_scale/log/model_validation/resid_size_sampling_grid.jpeg', width = 715*2, height = 495*2)

#plot grid points using residual size
plot(lc, legend=FALSE, col=c("coral3", "papayawhip", "forestgreen"), xaxt='n', yaxt='n',
     main = "Average SS Residual Size on Sampling Grid \nBlack Pos Blue Neg") 
par(xpd=TRUE)
...
#set up output plot
jpeg('output/spring_scale/log/model_validation/pdsi_ss_lc.jpeg', width = 715, height = 495)
...
#set up output plot
jpeg('output/spring_scale/log/model_validation/pdsi_ss_elev.jpeg', width = 715, height = 495)
...
#create index first
index <- c('(Intercept)', 'Herb', 'Evergreen', 'log(Rain)',
           'PDSI', 'PDSI^2', 'log(VPD)', 'PDSI:log(Rain)', 
           'PDSI^2:log(Rain)',
           'log(Rain):log(VPD)')
...
names(index) <- c('(Intercept)', 'lcherb', 'lcevergreen',
                  ...
                  'log_vpd_tavg_mean_jan_apr',
                  'poly(pdsi_mar_apr_min, 2, raw = TRUE)1:log_rain_mar_may',
                  'poly(pdsi_mar_apr_min, 2, raw = TRUE)2:log_rain_mar_may',
                  'log_rain_mar_may:log_vpd_tavg_mean_jan_apr')
...
#set up output plot
jpeg('output/spring_scale/log/model_validation/effect_dot_whisker.jpeg', width = 715, height = 495)
...
dotwhisker::dwplot(m_df,
  ...
  scale_colour_manual(name = 'Category', values = c('#EE6677', '#4477AA', '#228833')) +
