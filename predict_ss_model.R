#this code predicts future spring scale using future climate scenario data and the spring scale model built
#using the run_pirgd_model function
#the first part calculates average spring scale over the time periods using the average annual climate data
#the second part calculates annual spring scale over the time periods, which can then be used to calculate
#average spring scale (currently the preferred method in subsequent code)
...
foreach::foreach(i = 1:length(clim_files)) %dopar% {
  ...
  #load vpd data
  dat_clim$vpd <- read.csv(str_replace(clim_files[i], 'dat_avg', 'dat_avg_vpd'))$vpd
  ...
  index <- c('gdd_jan_apr', 'pdsi_mar_apr_min', 'rain_mar_may', 'lc', 'snow_oct_apr', 'vpd_tavg_mean_jan_apr')
  names(index) <- c('gdd', 'pdsi', 'rain', 'lc', 'snow', 'vpd')
  ...
  pred <- sapply(1:NROW(dat_clim), function(x){
    if(sum(is.na(dat_clim[x,])) == 0){
      predict(m_ss, dat_clim[x,], level = 0)
    } else{NA}
  })
  
  #create output raster of ss values
  pred_ras <- clim_ras
  pred_ras[] <- round(pred, 2)
  ...
  #write raster
  writeRaster(pred_ras, filename = str_c('wy_projections/ss/', out_name),
              format = 'GTiff')
...
foreach::foreach(i = 1:length(clim_files)) %dopar% {
  ...
  #load vpd data
  dat_clim$vpd <- read.csv(str_replace(clim_files[i], 'dat_', 'dat_vpd_'))$vpd
  ...
  for(j in 1:length(unique(dat_clim$year))){
    if(j == 1){
      pred_ras[] <- round(pred[min(which(dat_clim$year == unique(dat_clim$year)[j])):
                                          max(which(dat_clim$year == unique(dat_clim$year)[j]))], 2)
    }else{
      ras <- raster(clim_ras)
...
######################################################
###PROCESS MEAN Spring Scale BASED ON ANNUAL LAYERS###
######################################################

#create file list of pirgd projection rasters
files <- list.files('wy_projections/ss', glob2rx('ss_ann*.tif'),
                    full.names = T)
names <- list.files('wy_projections/ss', glob2rx('ss_ann*.tif'))

#loop through projection rasters
for(i in 1:length(files)){
  
  #load projection raster, calculate rounded mean, and write file
  brick(files[i]) %>% calc(., function(x) round(mean(x, na.rm = T), 2)) %>% 
    writeRaster(., filename = str_replace(files[i], 'ss_ann', 'ss_mean'),
